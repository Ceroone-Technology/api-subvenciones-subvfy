...
**Gotcha de teardown (comprobado, no hace falta tocar `conftest.py`)**: `alerta_ejecucion` y su tabla puente caen por la cascada de `alerta`, que el teardown borra antes que los usuarios y antes que las convocatorias. Tras la suite completa, el recuento de restos es cero en las seis tablas implicadas. Lo que sí importa: **las convocatorias que cree un test por esta vía deben usar `codigo_bdns_de_prueba()`**, o la limpieza no las reconoce.
