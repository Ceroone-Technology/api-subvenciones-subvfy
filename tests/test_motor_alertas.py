...
async def test_el_ciclo_marca_la_ejecucion_y_firma_como_sistema(gestor: Sesion) -> None:
...
async def test_el_evaluador_por_defecto_no_hace_nada_todavia(gestor: Sesion) -> None:
    """La consulta a la BDNS es de la tarea 2. Hasta entonces, evaluar una
    alerta no escribe nada más que su marca de ejecución."""
    alerta_id = await _crear_alerta(gestor.id)
    async with AsyncSessionLocal() as db:
        alerta = await db.get(Alerta, alerta_id)
        assert alerta is not None
